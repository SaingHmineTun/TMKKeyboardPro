...
public class ShanKeyboard {
    ...
    private ShanKeyboardService context;

    private ShanKeyboard(ShanKeyboardService context) {
        this.context = context;
    }

    private static ShanKeyboard instance;

    public static ShanKeyboard getInstance(ShanKeyboardService context) {
        if (instance == null) {
            instance = new ShanKeyboard(context);
        }
        return instance;
    }
    ...
    public String handleShanInputText(int primaryCode, InputConnection ic) {
        ...
        if ((primaryCode == MY_E || primaryCode == SH_E) && SharedPreferenceManagerKt.getHandWritingSystem(context)) {
        ...
        if (SharedPreferenceManagerKt.getHandWritingSystem(context))
    ...
    private boolean isConsonant(int code) {
        ...
        String separators = "ၵၶငၸသၺတထၼပၽၾမယရလဝႁဢၷꧠꩡႀၻꩪꧣၿꩮ";
